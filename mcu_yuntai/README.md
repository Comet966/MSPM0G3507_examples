...
### 直连串口接线（当前 demo，< 30 cm 短线）

不需要 RS485 收发器；每台电机各用一路独立 UART，TX/RX 直接连 PD42S1 A 端子，B 端子接 GND。

| 轴 | UART | TX 引脚 | RX 引脚 | 说明 |
|----|------|---------|---------|------|
| Pan  | UART1 | **PA8**  | **PA9**  | J14 拨到 PA9 侧 |
| Tilt | UART0 | **PA28** | **PA31** | BoosterPack pin 38/37 |
| — | — | GND | — | 两台 B 端子分别接 GND |

> **注意**：PA9 默认被 J14 跳线接到 PB23，需将 J14 拨到 **PA9** 侧。
...
### OLED SSD1306 接线（I2C，调试显示）

| MCU 信号 | MCU 引脚 | OLED 引脚 | 说明 |
|----------|----------|----------|------|
| I2C1 SCL | **PB2** | SCL | I2C 时钟，400kHz |
| I2C1 SDA | **PB3** | SDA | I2C 数据 |
| 3.3V | 3.3V | VCC | 供电 |
| GND | GND | GND | 共地 |

OLED 地址固定 `0x3C`（SA0 接 GND）；若你的模块 SA0 接 VCC 则地址为 `0x3D`，需修改 `oled.c` 中 `OLED_ADDR`。

### LP-MSPM0G3507 引脚汇总

| MCU 引脚 | LP 位置 | 用途 |
|----------|---------|------|
| PA8  | BP pin 4  | UART1 TX → Pan  PD42S1 A |
| PA9  | BP pin 3（J14 拨 PA9）| UART1 RX ← Pan  PD42S1 A |
| PA28 | BP pin 38 | UART0 TX → Tilt PD42S1 A |
| PA31 | BP pin 37 | UART0 RX ← Tilt PD42S1 A |
| PB2  | BP pin 9  | I2C1 SCL → OLED |
| PB3  | BP pin 10 | I2C1 SDA → OLED |
| GND  | BP pin 20/22 | 共地 |
| 3.3V | BP pin 1  | OLED 供电 |

---

### 长线 RS485 接线（> 30 cm，需收发器）

### 所需器件

| 器件 | 说明 |
|------|------|
| LP-MSPM0G3507 | 主控板（本工程所用） |
| MAX485 / SP3485 / SN65HVD3082 | RS485 差分收发器，任选其一，3.3V 兼容 |
| PD42S1 ×2 | Pan 轴（地址 0x01）+ Tilt 轴（地址 0x02）|
| 120Ω 电阻 ×1 | RS485 总线末端匹配电阻 |

### MCU → RS485 收发器接线

以 **MAX485 / SP3485**（8 脚，3.3V 供电）为例：

| MCU 信号 | MCU 引脚 | MAX485 引脚 | 说明 |
|----------|----------|------------|------|
| UART1_TX | **PA8** | DI（pin 4）| 串行数据发送 |
| UART1_RX | **PA9** | RO（pin 1）| 串行数据接收 |
| RS485_DE | **PB16** | DE（pin 3）+ /RE（pin 2）并接 | 方向控制（高=发，低=收）|
| GND | GND | GND（pin 5）| 共地 |
| 3.3V | 3.3V | VCC（pin 8）| 收发器供电 |

> `/RE` 与 `DE` 并联同一 GPIO（PB16）：高电平切换为发送，低电平切换为接收。

### RS485 总线 → 两台 PD42S1

```text
MAX485               PD42S1 #1 (Pan  0x01)   PD42S1 #2 (Tilt 0x02)
──────────────────────────────────────────────────────────────────
 A（pin 6） ────┬──── A 端子 ────────────────── A 端子
 B（pin 7） ────┴──── B 端子 ────────────────── B 端子
                                                │
...
- **A 接 A，B 接 B**；若出现无回复或乱码，交换两台 PD42S1 的 A/B 再试。
- 120Ω 终端电阻焊在**最远端**驱动器的 A/B 之间，线长 < 30 cm 可暂不装。
- 两台驱动器 GND 必须与 MCU GND **共地**。

### LP-MSPM0G3507 LaunchPad 引脚对照

| MCU 引脚 | LP 头排位置 | 说明 |
|----------|------------|------|
| PA8 | J3 pin 4（左侧内排）| UART1 TX |
| PA9 | J3 pin 3（左侧内排）| UART1 RX |
| PB16 | J4 外排（查板丝印）| RS485 DE 方向控制 |
| GND | J3 pin 2 / J4 pin 22 | 任一接地引出 |
| 3.3V | J3 pin 1 | MAX485 供电 |

> **J14 跳线（PA9 / PB23 切换）**：出厂默认 J14 将该排针连接到 **PB23**；  
> 使用 UART1 RX（PA9）时**必须将 J14 拨到 PA9 一侧**，否则接收不通。  
> 详见 [LP-MSPM0G3507 用户指南](https://www.ti.com/lit/slau873) Figure 2-1。

### 供电与安全

1. **MCU 3.3V** 与 **PD42S1 主电源（24~36V）** 独立供电，不得共电源。
2. **必须共地**：MCU GND → MAX485 GND → PD42S1 GND（统一参考电位）。
3. 先接好所有信号线再上电；建议先上 MCU/收发器电，再上驱动器主电。
4. 线长 > 1 m 时差分线请双绞，并安装终端电阻。
