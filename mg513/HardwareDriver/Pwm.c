...
/*
 * TB6612FNG dual-wheel driver over TIMG0 (2-channel, 20 kHz edge-aligned up).
 *
 * Wiring (11_PID_car pin map, direction pins split across two GPIO ports):
 ...
 *   STBY      = PB24          -> chip enable             (GPIOB)
 * Because the direction pins span GPIOA (AIN1/AIN2/BIN1) and GPIOB (BIN2/STBY),
 * every DL_GPIO call uses the per-pin *_PORT macro (SDK GPIO.md multi-port rule).
 *
...
{
    /* AIN1 and AIN2 are both on GPIOA (GPIO_Motor_AIN1_PORT == GPIO_Motor_AIN2_PORT). */
    if (signed_duty > 0) {                                    /* forward */
        ...
        DL_GPIO_clearPins(GPIO_Motor_AIN1_PORT, GPIO_Motor_AIN1_PIN | GPIO_Motor_AIN2_PIN);
    }
}

static inline void motor_right_dir(int16_t signed_duty)
{
    /* BIN1 is on GPIOA (PA7), BIN2 is on GPIOB (PB18) — different ports, so each
     * pin is driven with its own *_PORT macro (cannot be OR'd into one call). */
    if (signed_duty > 0) {                                    /* forward */
...
void PWMInit(void)
{
    DL_TimerG_startCounter(PWM_Motors_INST);
    PWM_SetDuty(0, 0);                                             /* coast + CC 0 */
    DL_GPIO_setPins(GPIO_Motor_STBY_PORT, GPIO_Motor_STBY_PIN);    /* release TB6612 from standby */
}

void PWMStop(void)
{
    PWM_SetDuty(0, 0);                                             /* coast both wheels */
    DL_GPIO_clearPins(GPIO_Motor_STBY_PORT, GPIO_Motor_STBY_PIN);  /* standby: outputs Hi-Z */
}
...
void PWM_SetDuty(int16_t left, int16_t right)
{
    ...
    DL_TimerG_setCaptureCompareValue(PWM_Motors_INST, magL, GPIO_PWM_Motors_C0_IDX);
    DL_TimerG_setCaptureCompareValue(PWM_Motors_INST, magR, GPIO_PWM_Motors_C1_IDX);
}
