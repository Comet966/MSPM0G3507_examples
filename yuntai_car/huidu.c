...
/* 直接读引脚统计"检测到黑线"的路数(0~5)，供 main 做关键点(全黑)识别。
 * 独立读取，不依赖 ISR 维护的 huidu_value[]，避免读写竞争。 */
uint8_t huidu_black_count(void)
{
    uint8_t n = 0;
    n += get_gpio_state(HUIDU_L2_PORT, HUIDU_L2_PIN);
    n += get_gpio_state(HUIDU_L1_PORT, HUIDU_L1_PIN);
    n += get_gpio_state(HUIDU_M_PORT,  HUIDU_M_PIN);
    n += get_gpio_state(HUIDU_R1_PORT, HUIDU_R1_PIN);
    n += get_gpio_state(HUIDU_R2_PORT, HUIDU_R2_PIN);
    return n;
}
