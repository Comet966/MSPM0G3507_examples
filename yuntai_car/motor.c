...
/* 整机运动模式，由 main 状态机控制（ISR 每 10ms 读取）：
 *   MOTION_STOP(0)     停车：强制目标速度=0，PID 主动刹停并锁住
 *   MOTION_TRACK(1)    循迹：adjust_motor() 根据灰度设目标速度
 *   MOTION_STRAIGHT(2) 直行：忽略灰度，两轮同速 g_straight_sps（穿越十字路口用）
 * 三态而非单开关：中间路口全黑时 adjust_motor 会停车，需切 STRAIGHT 才能开出去。*/
volatile uint8_t g_motion_mode  = 0;   /* MOTION_STOP */
volatile float   g_straight_sps = 0;   /* MOTION_STRAIGHT 时的目标轮速 mm/s */
...
void MOTOR_PID_INST_IRQHandler()
{
    ...
    case DL_TIMER_IIDX_LOAD:
        ...
        calculate_speed(2);
        if (!open_loop_test) {
            if (g_motion_mode == 1) {            // TRACK: 灰度循迹
                adjust_motor();
            } else if (g_motion_mode == 2) {     // STRAIGHT: 直行穿越
                motor_set_direction(1, 1);
                motor_set_direction(2, 1);
                target_speed_1 = g_straight_sps;
                target_speed_2 = g_straight_sps;
            } else {                             // STOP: 主动锁 0 速
                target_speed_1 = 0;
                target_speed_2 = 0;
            }
