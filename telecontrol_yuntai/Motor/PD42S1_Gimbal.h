...
/*
 * 摇杆速度控制参数。应用可在运行中修改此结构体并在下一帧生效，
 * 例如 cfg.pan_max_rpm = 40.0f 可将 PA8/PA9（下方 Pan）最高转速降至 40 RPM。
 * curve 为响应曲线指数：1=线性，2=平方，3=立方（推荐，中心更柔和）。
 */
typedef struct {
    uint16_t center;       /* ADC 摇杆中心值，12 位 ADC 通常为 2048 */
    uint16_t deadband;     /* 中心死区，单位为 ADC 计数 */
    float    pan_max_rpm;  /* PA8/PA9 下方 Pan 满偏最大转速，必须大于 0 */
    float    tilt_max_rpm; /* PA28/PA31 上方 Tilt 满偏最大转速，必须大于 0 */
    uint8_t  acceleration; /* PD42S1 加减速参数，0~200 */
    uint8_t  curve;        /* 1=线性，2=平方，3=立方 */
} pd42_joy_speed_config_t;
...
/* 摇杆速度层：只使用 0xF1 速度命令，居中/失联时下发 0 RPM，不使用立即刹车(0xFC)。 */
